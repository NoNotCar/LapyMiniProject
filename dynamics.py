...
def x_dot(s, f):
    aq1 = (l * m2 * np.sin(s[1]) * np.square(s[3]) + f + m2 * g * np.cos(s[1]) * np.sin(s[1])) / (m1 + m2 * (1 - np.cos(np.square(s[1]))))
...
cart_state_equations = {
    x: v,
    theta: omega,
    v: (l * m2 * sym.sin(theta) * omega**2 + force + m2 * g * sym.cos(theta) * sym.sin(theta)) / (m1 + m2 * (1 - sym.cos(np.square(theta)))),
